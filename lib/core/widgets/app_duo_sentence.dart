...
class DashedUnderlinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    while (x < size.width) {
      ...
      canvas.drawRRect(
        RRect.fromLTRBR(x, y, x + w, y + thickness, Radius.circular(thickness / 2)),
        paint,
      );
      x += dashWidth + dashGap;
    }
  ...
  bool shouldRepaint(covariant DashedUnderlinePainter old) =>
      old.color != color || old.dashWidth != dashWidth ||
          old.dashGap != dashGap || old.thickness != thickness;
}
...
class DuoWord extends StatefulWidget {
  const DuoWord({
    ...
    this.meanings = const [],
    this.tappable = true, // false → câu tiếng Việt: chữ trơn, không gạch, không popup
...
class _DuoWordState extends State<DuoWord> {
  final _ctrl = OverlayPortalController(); // OverlayPortalController là đối tượng đồng bộ, không cần dispose
  ...
  Widget build(BuildContext context) {
    ...
    final text = AnimatedDefaultTextStyle(
      ...
      child: Text(widget.word), // ⚠️ KHÔNG set style ở đây — phải kế thừa style animate
    );
      ...
      child: TapRegion(
        ...
        child: GestureDetector(
          ...
          child: CustomPaint(
            ...
            child: Padding(padding: const EdgeInsets.only(bottom: 6), child: text),
          ),
        ),
      ),
    );
      ...
      child: FractionalTranslation(
        ...
        child: TapRegion(
          ...
          child: ConstrainedBox(
            ...
            child: _MeaningPopup( // giữ nguyên class cũ của bạn
              meanings: widget.meanings.isEmpty ? const ['(chưa có dịch)'] : widget.meanings,
            ),
          ),
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          for (var i = 0; i < meanings.length; i++) ...[
            ...
            GestureDetector(                  // KHÔNG dùng InkWell: hết phụ thuộc Material
              onTap: () {/* chọn nghĩa */},
              child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                child: Text(
                  meanings[i],
                  style: const TextStyle(fontSize: 16, color: Color(0xFF4B4B4B)),
                ),
              ),
            ),
          ],
...
class AppDuoSentence extends StatefulWidget {
  const AppDuoSentence({
    ...
    this.revealAll = false, // true → câu tĩnh (màn "Chọn bản dịch đúng"), chữ hiện hết ngay
    this.tappable = true,
    this.onSpeak,
    this.onDone,
  });
          ...
          children: [
            if (widget.onSpeak != null)
              IconButton(
                ...
                icon: const Icon(Icons.volume_up_rounded, size: 32, color: Color(0xFF1CB0F6)),
              ),
